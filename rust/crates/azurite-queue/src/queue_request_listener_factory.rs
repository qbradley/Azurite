...
use crate::{
    authentication::{
        AccountSASAuthenticator, QueueSASAccessPolicySource, QueueSASAuthenticator,
        QueueSharedKeyAuthenticator, QueueTokenAuthenticator,
    },
    context::QueueStorageContext,
    generated::{
        artifacts::models::{AccessPolicy, GeneratedValue},
        context::Context,
        handlers::{
            IHandlers, IMessageIdHandler, IMessagesHandler, IQueueHandler, IServiceHandler,
        },
        i_request::{
    ...
    handlers::{
        base_handler::{BaseHandler, SharedExtentStore, SharedLogger, SharedQueueMetadataStore},
        message_id_handler::MessageIdHandler,
        messages_handler::MessagesHandler,
        queue_handler::QueueHandler,
        service_handler::ServiceHandler,
    },
    middlewares::{
        createQueueStorageContextMiddleware, internalQueueStorageContextMiddleware,
        AuthenticationMiddleware, AuthenticationMiddlewareFactory, BoxedMiddlewareError,
        CorsRequestMiddleware, OptionsHandlerMiddleware, QueueStorageContextMiddlewareOptions,
        SharedAuthenticator, TelemetryMiddleware, TelemetryMiddlewareFactory,
    },
    utils::constants::DEFAULT_QUEUE_CONTEXT_PATH,
};
...
struct RequestListenerHandlers {
    ...
    messages: Arc<MessagesHandler>,
    message_id: Arc<MessageIdHandler>,
}
...
#[derive(Clone)]
struct MetadataStoreQueueSASAccessPolicySource {
    metadata_store: SharedQueueMetadataStore,
}

#[async_trait]
impl QueueSASAccessPolicySource for MetadataStoreQueueSASAccessPolicySource {
    async fn get_queue_access_policy(
        ...
        let queue = self
            .metadata_store
            .getQueue(account, queue, Some(context))
            .await
            .ok()?;
        let acl = queue.queueAcl?;

        for signed_identifier in acl {
            let id = signed_identifier
                .get("id")
                .or_else(|| signed_identifier.get("Id"))
                .and_then(GeneratedValue::as_string);
            if id.as_deref() != Some(identifier) {
                continue;
            }

            return signed_identifier
                .get("accessPolicy")
                .or_else(|| signed_identifier.get("AccessPolicy"))
...
struct RequestListenerState {
    ...
    queueStorageContextOptions: QueueStorageContextMiddlewareOptions,
    ...
    async fn handle_request(&self, request: Request<Body>) -> Response<Body> {
        ...
        let mut pending_error: Option<BoxedMiddlewareError> =
            internalQueueStorageContextMiddleware(
                ...
                &request_path,
                self.logger.as_ref(),
                self.queueStorageContextOptions.skipApiVersionCheck,
                self.queueStorageContextOptions.disableProductStyleUrl,
            )
            .err()
        ...
        if pending_error.is_none() {
            if let Err(error) = self
                ...
                .apply(&queue_context, &generated_request, &generated_response)
                .await
            {
...
impl QueueRequestListenerFactory {
    ...
    pub fn createRequestListener(&self) -> RequestListener {
        ...
        let handlers = Arc::new(RequestListenerHandlers {
            ...
            queue: Arc::new(QueueHandler::new(baseHandler.clone())),
            messages: Arc::new(MessagesHandler::new(baseHandler.clone())),
            message_id: Arc::new(MessageIdHandler::new(baseHandler)),
        });

        let authenticationMiddlewareFactory =
            AuthenticationMiddlewareFactory::new(Arc::clone(&logger));
        let access_policy_source = Arc::new(MetadataStoreQueueSASAccessPolicySource {
            metadata_store: Arc::clone(&self.metadataStore),
        });
        let mut authenticators: Vec<SharedAuthenticator> = vec![
            Arc::new(QueueSharedKeyAuthenticator::new(
            ...
            Arc::new(QueueSASAuthenticator::new(
                Arc::clone(&self.accountDataStore),
        ...
        if let Some(oauth) = self.oauth {
            authenticators.push(Arc::new(QueueTokenAuthenticator::new(
                Arc::clone(&self.accountDataStore),
                oauth,
                Arc::clone(&logger),
            )) as SharedAuthenticator);
        ...
        let telemetryMiddlewareFactory =
            TelemetryMiddlewareFactory::new(DEFAULT_QUEUE_CONTEXT_PATH.to_string());
        ...
        let state = Arc::new(RequestListenerState {
            ...
            logger,
            handlers,
            queueStorageContextOptions: createQueueStorageContextMiddleware(
...
impl IRequestListenerFactory for QueueRequestListenerFactory {
    fn createRequestListener(&self) -> RequestListener {
        QueueRequestListenerFactory::createRequestListener(self)
    }
}

async fn queue_request_listener(
