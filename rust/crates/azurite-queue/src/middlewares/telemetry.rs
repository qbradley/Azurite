...
fn build_telemetry_context(context: &Context) -> TelemetryContext {
    TelemetryContext {
        serviceType: TelemetryServiceType::Queue,
...
mod tests {
    ...
    #[test]
    fn telemetry_context_uses_queue_service_type() {
        let context = Context::default();
        let mut request = GeneratedHttpRequest::new(
            HttpMethod::GET,
            "http://127.0.0.1/devstoreaccount1/queue/messages",
        ...
        assert!(matches!(
            ...
            azurite_common::telemetry::TelemetryServiceType::Queue
        ));
        assert_eq!(
            telemetry
                .request
