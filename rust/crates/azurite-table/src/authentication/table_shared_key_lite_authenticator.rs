...
pub struct TableSharedKeyLiteAuthenticator {
    dataStore: Arc<dyn IAccountDataStore + Send + Sync>,
    _logger: Arc<dyn ILogger + Send + Sync>,
}

impl TableSharedKeyLiteAuthenticator {
    pub fn new(
        dataStore: Arc<dyn IAccountDataStore + Send + Sync>,
    ...
    fn build_string_to_sign(
        ...
        [self.date_value_for_signature(req)].join("\n")
            + "\n"
...
#[async_trait]
impl IAuthenticator for TableSharedKeyLiteAuthenticator {
    async fn validate(
        &self,
        req: &GeneratedHttpRequest,
        content: &Context,
        ...
        let authHeaderValue = match req.getHeader(AUTHORIZATION) {
            Some(value) if value.starts_with("SharedKeyLite") => value,
            _ => return Ok(None),
        };
        ...
        if authHeaderValue == format!("SharedKeyLite {account}:{signature1}") {
            return Ok(Some(true));
        }

        if let Some(key2) = accountProperties.key2.as_deref() {
            let signature2 = computeHMACSHA256(&stringToSign, key2);
            if authHeaderValue == format!("SharedKeyLite {account}:{signature2}") {
                return Ok(Some(true));
            }
        }
                ...
                .unwrap_or(false)
        {
            ...
            if authHeaderValue == format!("SharedKeyLite {account}:{signature1_secondary}") {
                return Ok(Some(true));
            }

            if let Some(key2) = accountProperties.key2.as_deref() {
                let signature2_secondary = computeHMACSHA256(&stringToSign_secondary, key2);
                if authHeaderValue == format!("SharedKeyLite {account}:{signature2_secondary}") {
                    return Ok(Some(true));
                }
            }
        }
