...
impl GreaterThanEqualNode {
    pub fn new(left: Box<dyn IQueryNode>, right: Box<dyn IQueryNode>) -> Self {
        Self {
            inner: BinaryOperatorNode::new("ge", left, right),
        }
    }
}

impl IQueryNode for GreaterThanEqualNode {
    ...
    fn evaluate(&self, context: &dyn IQueryContext) -> Result<QueryValue, QueryError> {
        ...
        {
            return Ok(QueryValue::Bool(matches!(
                result,
                QueryComparison::Greater | QueryComparison::Equal
            )));
        }
            ...
            .compare(context, self.inner.left.as_ref())?
        {
            return Ok(QueryValue::Bool(matches!(
                result,
                QueryComparison::Less | QueryComparison::Equal
            )));
        }
