...
impl NotEqualsNode {
    pub fn new(left: Box<dyn IQueryNode>, right: Box<dyn IQueryNode>) -> Self {
        Self {
            inner: BinaryOperatorNode::new("ne", left, right),
        }
    }
}

impl IQueryNode for NotEqualsNode {
    ...
    fn evaluate(&self, context: &dyn IQueryContext) -> Result<QueryValue, QueryError> {
            ...
            .compare(context, self.inner.right.as_ref())?
        {
            return Ok(QueryValue::Bool(!matches!(
                result,
                QueryComparison::Equal | QueryComparison::Nan
            )));
        }
        ...
        Ok(QueryValue::Bool(
            !left.strict_equal(&right) && !left.is_undefined() && !right.is_undefined(),
        ))
    }
