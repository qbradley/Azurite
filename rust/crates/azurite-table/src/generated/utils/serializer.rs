...
pub async fn serialize<R: IResponse, L: ILogger + ?Sized>(
    ...
    if let Some(bodyMapper) = &responseSpec.bodyMapper {
        if bodyMapper.r#type.name == "Stream" {
            ...
            // Apply handler's content type for stream responses
            if let Some(ct) = &handlerResponse.contentType {
                res.setContentType(Some(ct.clone()));
            }
        } else {
            let body = handlerResponse.body_value().to_json_value();
            // If the handler explicitly set a JSON content type, serialize as JSON
            // even if the spec says isXML (table operations use JSON via OData)
            let use_xml = spec.isXML
                && !handlerResponse
                    .contentType
                    .as_deref()
            ...
            if use_xml {
                ...
                if let Some(ct) = &handlerResponse.contentType {
                    res.setContentType(Some(ct.clone()));
                } else if res.getHeader("content-type").is_none() {
    ...
    } else if let Some(ct) = &handlerResponse.contentType {
        // No body mapper, but handler set content type (e.g., for empty body responses)
...
fn resolve_additional_properties(_mapper: &Mapper) -> Option<&Mapper> {
    // Table mappers don't use additionalProperties
    None
}
