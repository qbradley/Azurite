...
mod tests {
    ...
    use super::PreflightMiddlewareFactory;
    ...
    #[test]
    fn check_headers_supports_prefix_rules() {
        let factory = PreflightMiddlewareFactory::new(Arc::new(TestLogger));

        assert!(factory.checkHeaders("x-ms-meta-name,x-ms-version", "x-ms-meta-*,x-ms-version"));
        assert!(!factory.checkHeaders("x-ms-foo", "x-ms-bar-*"));
    }

    #[test]
    fn get_exposed_headers_merges_prefix_and_simple_rules() {
        let factory = PreflightMiddlewareFactory::new(Arc::new(TestLogger));
        let headers = vec![
            String::from("x-ms-request-id"),
            String::from("x-ms-meta-name"),
            String::from("content-length"),
        ];

        let exposed = factory.getExposedHeaders(&headers, "x-ms-meta-*,x-ms-request-id,Date");

        assert_eq!(exposed, "x-ms-request-id,x-ms-meta-name,Date");
    }
}
