...
use crate::{
    ...
    persistence::{IBlobMetadataStore, ServicePropertiesModel},
...
impl PreflightMiddlewareFactory {
    ...
    fn checkOrigin(&self, origin: Option<&str>, allowedOrigin: &str) -> bool {
        ...
        for corsOrigin in allowedOrigin.split(',') {
            let corsOrigin = corsOrigin.trim().to_ascii_lowercase();
            if corsOrigin.contains('*') {
                if wildcard_regex(&corsOrigin)
                    .map(|regex| regex.is_match(&origin.trim().to_ascii_lowercase()))
                    .unwrap_or(false)
                {
                    return true;
                }
            } else if origin.trim().eq_ignore_ascii_case(&corsOrigin) {
                return true;
            }
        }
        false
