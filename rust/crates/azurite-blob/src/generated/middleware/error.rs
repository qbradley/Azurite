use crate::errors::{NotImplementedError, NotImplementedinSQLError, StorageError};
...
pub fn error_middleware<RQ: IRequest, RS: IResponse, L: ILogger + ?Sized>(
    ...
    if let Some(err) = err.downcast_ref::<NotImplementedinSQLError>() {
        logger.error(
            "ErrorMiddleware: Received a NotImplementedinSQLError, fill error information to HTTP response",
            context.contextId().as_deref(),
        );
        write_error_response(
            req,
