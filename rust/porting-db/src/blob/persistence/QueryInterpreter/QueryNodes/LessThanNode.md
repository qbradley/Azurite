# Porting Record — `src/blob/persistence/QueryInterpreter/QueryNodes/LessThanNode.ts`

## File info
- Source path: `src/blob/persistence/QueryInterpreter/QueryNodes/LessThanNode.ts`
- Source lines: `26`
- Source type: `handwritten`
- Rust target (per `PORTING-ORDER.md`): `azurite-blob/src/persistence/query_interpreter/query_nodes/less_than_node.rs`
- Crate: `azurite-blob`
- Module: `persistence::query_interpreter::query_nodes::less_than_node`
- Phase: `10.4`
- Status: `not_started`

## Exported API
### Default class `LessThanNode`
...
## Special handling
- `LessThanNode.ts` evaluates both child nodes and only succeeds when both witness values are defined and the left value is lexicographically less than the right.
...
- `LessThanNode.ts:9-25` also leaves `evaluate()` typed as `any`.
