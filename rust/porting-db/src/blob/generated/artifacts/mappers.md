# Porting Record — `src/blob/generated/artifacts/mappers.ts`

## File info
- Source path: `src/blob/generated/artifacts/mappers.ts`
- Source lines: `7394`
- Source type: `autorest-generated`
- Rust target (per `PORTING-ORDER.md`): `azurite-blob/src/generated/artifacts/mappers.rs`
- Crate: `azurite-blob`
...
|---|---|---|
| `msRest.CompositeMapper` consts | `static MapperSpec` descriptors or derive metadata | Need to preserve `serializedName`, `xmlName`, `className`, and nested `modelProperties` exactly. |
| `Composite`/`Sequence`/`Dictionary` mapper types | Rust enum describing mapper shape | Generated serializer/deserializer branch on mapper type names. |
| `DateTimeRfc1123`, `String`, numeric mapper names | typed serializer primitives | Mapper `type.name` strings are runtime-dispatched today. |
## `any` hotspots
- No runtime `any` usages in this file beyond literal message text/comments.

## Generated-code notes
- Export inventory: 123 composite mappers (50 body/option/headerless models and 73 `*Headers` mappers).
- These descriptors are the schema authority used by `spec.serializer.deserialize()` / `serialize()`. Rust should not throw them away even if many fields are representable as plain structs, because future regenerated diffs land here first.
- `serializedName`, `xmlName`, `headerCollectionPrefix`, `required`, and nested `modelProperties` values are all wire-contract data.

## Middleware chain ordering
- Deserializer and serializer both depend on these mappers via the shared `msRest.Serializer` instance created in `specifications.ts`.
...
- Diff this file together with `models.ts` because mapper `className` and model declarations are paired 1:1.
- If a mapper changes from scalar to sequence/dictionary/composite, update Rust serializer branching rather than forcing it through the old path.
