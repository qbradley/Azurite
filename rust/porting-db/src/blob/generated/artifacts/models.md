# Porting Record — `src/blob/generated/artifacts/models.ts`

## File info
- Source path: `src/blob/generated/artifacts/models.ts`
- Source lines: `8933`
- Source type: `autorest-generated`
- Rust target (per `PORTING-ORDER.md`): `azurite-blob/src/generated/artifacts/models.rs`
- Crate: `azurite-blob`
...
- Full exported declarations copied below so Aragorn can diff future autorest regenerations mechanically.
...
|---|---|---|
| `export interface ...` model declarations | `pub struct ...` | Preserve field names, optionality, array nesting, and XML comments/semantics close to TS for future diffs. |
| `export enum ...` string enums | `enum` with exact string wire values | Generated enums encode REST wire contracts; do not normalize names/values. |
| `Response = A & B & { statusCode: ... }` | flattened response struct or explicit composition wrapper | Intersection types should remain mechanically traceable to the original pieces. |
| `[property: string]: string | any` and other `any` fields | `HashMap<String, serde_json::Value>` or explicit compatibility enum | Autorest leaves some extension points intentionally untyped. |
| `NodeJS.ReadableStream` response bodies | `BodyStream` / `dyn AsyncRead` | Streaming responses must stay streaming rather than being buffered. |
## `any` hotspots
- `182: * to valid TS constraints we have modeled this as a union of \`string | any\`.`
- `184: [property: string]: string | any;`
- `591: parquetTextConfiguration?: any;`
- `683: * Optional.  Version 2019-07-07 and newer.  If true, prevents any request from specifying a`

## Generated-code notes
- Export inventory counts: 190 interfaces, 100 enums, 72 exported type aliases.
- The file is organized in three major families: request/response body models, `*OptionalParams` request-option interfaces, and `*Headers`/`*Response` result contracts.
- Response type aliases use intersection types (`BodyModel & Headers & { statusCode: ... }`) to define the API contract the generated serializer expects.
- Known `any` escape hatches are limited and visible: open-ended metadata bags, `parquetTextConfiguration?: any`, and similar extension points.

## Middleware chain ordering
- Handler interfaces consume the `*OptionalParams` and `*Response` types; serializer/deserializer resolve payload shapes through `mappers.ts` and `specifications.ts` rather than directly through this file.
